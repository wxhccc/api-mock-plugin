...
function requestHanlder (req, res, next) {
  const { MOCKDIR, index: INDEX, minDelay, maxDelay, suffix } = this.options
  res.header('Access-Control-Allow-Origin', '*')
  const { path: urlPath, method } = req
  ...
  if (!resData) {
    if (urlPath && urlPath !== '/') {
      ...
      res.sendFile(fileDir, (err) => {
        delayRes(() => { err && res.status(err.status).end() })
      })
    }
  } else {
...
function fileReader (req, res, next) {
  const { options: { MOCKDIR }, fileDirMap } = this
  fs.readFile(path.join(MOCKDIR, fileDirMap[req.path]), 'utf-8', (error, result) => {
    res.send(result)
  })
}
/** export plugin main function **/
export default function (mockDir, options = {}) {
  const opts = Object.assign({ index: 'index.js', routes: '/api-mock', minDelay: 0, maxDelay: 1000, suffix: 'json' } , options, { MOCKDIR: typeof mockDir === 'string' ? mockDir : null })
  const { MOCKDIR, appHanlder, readFiles, routes } = opts
