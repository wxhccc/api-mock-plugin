...
function log(message, site) {
  var haslisteners = eventListenerCount$1(process, 'deprecation') !== 0; // abort early if no destination
...
var statuses = status; // status code to message map

status.STATUS_CODES = codes$2; // array of status codes

status.codes = populateStatusesMap(status, codes$2); // status codes for redirects
...
function getRawBody(stream, options, callback) {
  ...
  var limit = bytes_1.parse(opts.limit); // convert the expected length to an integer
...
function normalizeType(value) {
  // parse the type
  var type = mediaTyper.parse(value); // remove the parameters
...
function json(options) {
  ...
  return function jsonParser(req, res, next) {
    ...
    debug$1('content-type %j', req.headers['content-type']); // determine if request should be parsed
...
function raw(options) {
  ...
  return function rawParser(req, res, next) {
    ...
    debug$2('content-type %j', req.headers['content-type']); // determine if request should be parsed
...
function text(options) {
  ...
  return function textParser(req, res, next) {
    ...
    debug$3('content-type %j', req.headers['content-type']); // determine if request should be parsed
    ...
    var charset = getCharset$1(req) || defaultCharset; // read
...
function urlencoded(options) {
  ...
  return function urlencodedParser(req, res, next) {
    ...
    debug$4('content-type %j', req.headers['content-type']); // determine if request should be parsed
...
function index (mockDir, options = {}) {
  ...
  return app => {
    try {
      ...
      app.use(routes, bodyParser_1.json(), bodyParser_1.urlencoded({
        extended: false
      }), workRequestHanlder);
